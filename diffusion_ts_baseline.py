...
class DiffusionTS(nn.Module):
    ...
    def sample(self, x_rest: torch.Tensor, T_pred: int, num_steps: int = 50):
        ...
        # Clip range for normalized data to avoid numerical explosion and huge MSE
        clip_min, clip_max = -4.0, 4.0
        indices = list(range(self.num_timesteps - 1, -1, -max(1, self.num_timesteps // num_steps)))
        if indices[-1] != 0:
            indices.append(0)
...
def main():
    ...
    for ep in range(1, args.epochs + 1):
        tr_loss = train_epoch(model, train_loader, opt, args.device, args.max_grad_norm)
