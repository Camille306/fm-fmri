# STAGIN model from https://github.com/egyptdj/stagin (NeurIPS 2021)
...
class ModelSTAGIN(nn.Module):
    def __init__(
        ...
        if cls_token == "sum":
            ...
            raise ValueError(f"Unknown cls_token: {cls_token}")
        if readout == "garo":
        ...
        self.token_parameter = (
            nn.Parameter(torch.randn([num_layers, 1, 1, hidden_dim])) if cls_token == "param" else None
        )
    ...
    def _collate_adjacency(self, a, sparsity, sparse=True):
        ...
        return torch.sparse.FloatTensor(
            _i, _v, (a.shape[0] * a.shape[1] * a.shape[2], a.shape[0] * a.shape[1] * a.shape[3])
        )
    ...
    def forward(self, v, a, t, sampling_endpoints):
        ...
        for layer, (G, R, T, L) in enumerate(zip(self.gnn_layers, self.readout_modules, self.transformer_modules, self.linear_layers)):
            ...
            if self.token_parameter is not None:
                h_readout = torch.cat([h_readout, self.token_parameter[layer].expand(-1, h_readout.shape[1], -1)])
            ...
            reg_ortho += (
                (matrix_inner / matrix_inner.max(-1)[0].unsqueeze(-1) - torch.eye(num_nodes, device=matrix_inner.device))
                .triu()
                .norm(dim=(1, 2))
                .mean()
            )
...
class Percentile(torch.autograd.Function):
    ...
    def backward(self, grad_output):
        ...
        grad_input = torch.zeros((in_argsort.size()), device=grad_output.device)
