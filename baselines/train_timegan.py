...
def train_joint(model, loader, g_optimizer, d_optimizer, device, prediction_length,
                lambda_embed=10.0, lambda_supervise=1.0, lambda_adv=0.1, lambda_psd=0.0):
    model.train()
    model.embedder.train()
    model.recovery.train()
    model.supervisor.train()
    ...
    for batch in tqdm(loader, desc="Joint Train", leave=False):
        ...
        g_loss_psd = frequency_loss_torch(x_rec, x_task) if lambda_psd > 0 else torch.tensor(0.0, device=x_task.device)

        h_sup = model.supervise(h_fake[:, :-1, :])                       # (B,T-1,H)
        g_loss_sup = criterion(h_sup, h_fake[:, 1:, :])

        g_loss = lambda_adv * g_loss_adv + lambda_embed * g_loss_embed + lambda_supervise * g_loss_sup + lambda_psd * g_loss_psd
...
def main():
    ...
    parser.add_argument("--freq_loss_weight", type=float, default=0.0, help="Weight for PSD auxiliary loss (lambda_psd)")

    parser.add_argument("--device", type=str, default=None)
    parser.add_argument("--num_workers", type=int, default=1)
    parser.add_argument("--save_dir", type=str, default="./checkpoints_timegan_fmts_eval")
    parser.add_argument("--eval_only", action="store_true", help="Load best checkpoint, run test eval only (no training)")
    ...
    if args.eval_only:
        ...
        print("\nEvaluating BEST model on test set (eval_only, subject-level dedup)...")
        ...
        print("\n" + "=" * 70)
        print("TimeGAN TEST (eval_only, Subject-level, Deduplicated) [BEST CKPT]")
        print("=" * 70)
    ...
    for ep in range(1, args.epochs + 1):
        g_loss, d_loss = train_joint(
            ...
            lambda_psd=args.freq_loss_weight,
        )
