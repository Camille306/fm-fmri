"""
Diffusion-TS Baseline for Rest-to-Task fMRI Prediction

Conditional diffusion model for time series: rest-conditioned noise prediction.
- Forward process: x_t = sqrt(alpha_bar_t) * x_0 + sqrt(1 - alpha_bar_t) * eps
- Train: predict eps given (x_t, t, rest_ctx) with MSE
- Sample: reverse diffusion from x_T ~ N(0,I) using predicted noise (DDIM-style steps)
...
# ======================================================================
# Diffusion-TS: rest encoder + time embedding + noise prediction net
# ======================================================================
...
class DiffusionTS(nn.Module):
    def __init__(self, v_dim: int, num_timesteps: int = 1000, rest_hidden: int = 256, ctx_dim: int = 256, t_dim: int = 128):
        super().__init__()
        self.v_dim = v_dim
        ...
        self.noise_net = NoisePredNet(v_dim=v_dim, ctx_dim=ctx_dim, t_dim=t_dim)
        # Linear schedule: beta_1=1e-4, beta_T=0.02
    ...
    def forward_noise(self, t: torch.LongTensor, x_t: torch.Tensor, x_rest: torch.Tensor):
        rest_ctx = self.rest_enc(x_rest)
        t_emb = self.t_embed(t.float() / self.num_timesteps)
        return self.noise_net(x_t, rest_ctx, t_emb)
    ...
    def train_step_loss(self, x_0: torch.Tensor, x_rest: torch.Tensor):
        ...
        eps_pred = self.forward_noise(t, x_t, x_rest)
    ...
    def sample(self, x_rest: torch.Tensor, T_pred: int, num_steps: int = 50):
        ...
        for k, i in enumerate(indices):
            ...
            eps_pred = self.forward_noise(t_batch, x, x_rest)
            sqrt_ab = self.sqrt_alpha_bar[i].view(1, 1, 1)
            sqrt_1_ab = self.sqrt_one_minus_alpha_bar[i].view(1, 1, 1)
            x_0_pred = (x - sqrt_1_ab * eps_pred) / (sqrt_ab + 1e-8)
            x_0_pred = torch.clamp(x_0_pred, clip_min, clip_max)
            # If next step is t=0, return denoised x_0 now (avoid re-noising then trivial "denoise")
            i_prev = indices[k + 1] if k + 1 < len(indices) else 0
            if i_prev == 0:
                return x_0_pred
            if i == 0:
                return x_0_pred
            sqrt_ab_prev = self.sqrt_alpha_bar[i_prev].view(1, 1, 1)
            sqrt_1_ab_prev = torch.sqrt(1.0 - self.alpha_bar[i_prev]).view(1, 1, 1)
            x = sqrt_ab_prev * x_0_pred + sqrt_1_ab_prev * eps_pred
            x = torch.clamp(x, clip_min, clip_max)
        return x

    def sample_deterministic(self, x_rest: torch.Tensor, T_pred: int, num_steps: int = 10):
        """Same as sample but no @torch.no_grad(). Differentiable for PSD auxiliary loss."""
        B, L, V = x_rest.shape
        x = torch.randn(B, T_pred, V, device=x_rest.device)
        clip_min, clip_max = -4.0, 4.0
        step = max(1, self.num_timesteps // num_steps)
...
def main():
    ...
    p.add_argument("--freq_aux_steps", type=int, default=10, help="Reverse steps for PSD loss")
    p.add_argument("--device", type=str, default=None)
    p.add_argument("--num_workers", type=int, default=1)
    p.add_argument("--save_dir", type=str, default="./checkpoints_diffusion_ts")
    p.add_argument("--eval_only", action="store_true", help="Load best checkpoint, run test eval only (no training)")
    args = p.parse_args()
    ...
    model = DiffusionTS(v_dim=V, num_timesteps=args.num_timesteps, rest_hidden=args.rest_hidden, ctx_dim=args.ctx_dim).to(args.device)
    opt = Adam(model.parameters(), lr=args.lr, weight_decay=args.weight_decay)
    sched = CosineAnnealingLR(opt, T_max=args.epochs)

    best_path = os.path.join(args.save_dir, "best_diffusion_ts.pth")
    if args.eval_only:
        ...
        print("=" * 60)
        print("Diffusion-TS TEST (eval_only, Subject-level dedup)")
        print(f"MSE: {test_metrics['mse']:.6f} ± {test_metrics['mse_std']:.6f}")
        print(f"MAE: {test_metrics['mae']:.6f} ± {test_metrics['mae_std']:.6f}")
        print(f"PSD (abs power spectrum diff): {test_metrics['freq_diff']:.6f} ± {test_metrics['freq_diff_std']:.6f}")
        ...
        save_closest_subject_visualizations(
            ...
            model_name="Diffusion-TS",
            fs=0.72,
        )
        test_results_path = os.path.join(args.save_dir, "test_results.txt")
        with open(test_results_path, "w", encoding="utf-8") as f:
            f.write("Diffusion-TS TEST (Subject-level dedup) [BEST CKPT]\n")
            f.write("=" * 60 + "\n")
            ...
            f.write(f"PSD (abs power spectrum diff, mean ± std): {test_metrics['freq_diff']:.6f} ± {test_metrics['freq_diff_std']:.6f}\n")
    ...
    print("=" * 60)
    print("Diffusion-TS TEST (subject-level dedup)")
    print(f"MSE: {test_metrics['mse']:.6f} ± {test_metrics['mse_std']:.6f}")
    print(f"MAE: {test_metrics['mae']:.6f} ± {test_metrics['mae_std']:.6f}")
    print(f"PSD (abs power spectrum diff): {test_metrics['freq_diff']:.6f} ± {test_metrics['freq_diff_std']:.6f}")
    print(f"FC sim: {test_metrics['fc_similarity']:.6f} ± {test_metrics['fc_similarity_std']:.6f}")
